...
def edit():
    ...
    register_start = input("when does he/she start thier shift please write @ following with thier time:")
    register_end = input("when does he/she end thier shift please write @ following with thier time:")
    ...
    if registerNother == "yes":
        ...
        connection.execute("INSERT OR REPLACE INTO employee_personal (firstName,lastName,email,userName,Age,Wage,position)VALUES (?,?,?,?,?,?,?,?)",(register_firstName,register_lastName,register_email,register_userName,register_age,register_Wage,register_position))
        connection.commit()
        connection.execute("INSERT OR REPLACE INTO application_infomation (Username,password,email)VALUES (?,?,?,?)",(userID,username,password, register_email))
        ...
        connection.execute("INSERT OR REPLACE INTO employee_pay (UserID,Username,lastName,weeklyPay,wage,age)VALUES (?,?,?,?,?,?)",(userID,username,register_lastName,weeklyPay(), calculateWage(),register_age))
        connection.commit()
        connection.execute("INSERT OR REPLACE INTO employee_shifts (UserID,Positions,startingTime,endingTime,shiftsPerWeek) VALUES (?,?,?,?,?)",(userID,register_position,register_start,register_end,register_spw))
        connection.commit()
        connection.execute("INSERT OR REPLACE INTO late_arrivals (UserID,employeeName,startingTime,arrivalTime,lateBy,lateAmt) VALUES (?,?,?,?,?)",(userID,register_firstName,register_start,arrivalTime(),late_factor,random.randint(0,15)))
        connection.commit()
        number =  fake.phone_number()
        connection.execute("INSERT OR REPLACE INTO emergency_contact (UserID,Email,phoneNo)VALUES (?,?,?,)",(userID,register_email,number))
        connection.commit()
        connection.execute("INSERT OR REPLACE INTO normal_contact (UserID,Email,adress)VALUES (?,??,)",(userID,register_email,faker.address()))
        connection.commit()
        edit()

    if registerNother == 'no':
        connection.execute("INSERT OR REPLACE INTO employee_personal (firstName,lastName,email,userName,Age,position)VALUES (?,?,?,?,?,?)",
        (register_firstName,register_lastName,register_email,register_userName,register_age,register_position))
        connection.commit()
        ...
        connection.execute('''
            ...
            VALUES (?,?,?,?,?,?)
            ''',
            (userID,register_firstName,register_start,arrivalTime(),late_factor,random.randint(0,15)))
        connection.commit()
        number =  fake.phone_number()
        adress = fake.address()
        connection.execute("INSERT OR REPLACE INTO emergency_contact (UserID,Email,phoneNo) VALUES (?,?,?)",(userID,register_email,number))
